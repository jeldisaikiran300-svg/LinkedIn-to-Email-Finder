...
This project takes a list of LinkedIn profile URLs and finds their associated verified emails and profile data (Name, Title, Company) using the [Apollo.io API](https://www.apollo.io/). 
...
## Prerequisites
- Python 3.8+
- [Google Chrome](https://www.google.com/chrome/) (if using the Selenium Scraper)
...
### How it works
1. **Reads `input.csv`**: Iterates through each LinkedIn profile URL.
2. **Calls Apollo API**: Hits `POST /api/v1/people/match` with the URL.
3. **Saves Data**: Exports the resulting match (Name, Title, Company, Email) to `output.csv`.
